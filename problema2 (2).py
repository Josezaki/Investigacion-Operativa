...
def weighted_metrics(pi: np.ndarray):
    ...
    Retorna:
      - df_states: DataFrame con métricas por estado
      - resumen:  diccionario con métricas ponderadas
    """
    n_states = pi.shape[0]
    data = []
    
    for i in range(n_states):
        k = states[i] # Número de cajeros (servidores) en el estado i
        
        # Parámetros para la cola M/M/k
        metrics = mmk_metrics(lamb, mu_per_cajero, k)
        
        # Añadir la probabilidad estacionaria (π) y el número de cajeros (k)
        metrics['pi'] = pi[i]
        metrics['k'] = k
